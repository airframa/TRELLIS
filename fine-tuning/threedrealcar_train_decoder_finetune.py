...
def main_worker(local_rank, cfg):
    ...
    # Load dataset
    dataset = getattr(datasets, cfg.dataset.name)(roots=cfg.data_dir, **cfg.dataset.args)
    print(f"[Worker {local_rank}] Training dataset loaded: {len(dataset)} samples")

    # Load validation dataset if specified
    val_dataset = None
    if cfg.get('val_dir'):
        val_dataset = getattr(datasets, cfg.dataset.name)(roots=cfg.val_dir, **cfg.dataset.args)
        print(f"[Worker {local_rank}] Validation dataset loaded: {len(val_dataset)} samples")

    # Build models
    # Build decoder
    decoder_class = getattr(models, cfg.models.decoder.name)
    decoder = decoder_class(**cfg.models.decoder.args)
    ...
    # Create a dummy encoder (will be replaced in trainer)
    ...
    if rank == 0:
        print(f"\nModels initialized:")
        print(f"Encoder: Dummy (using pre-computed latents)")
        ...
        print(f"\nDecoder structure:")
        print(f"  - Has blocks: {hasattr(decoder, 'blocks')}")
    ...
    # Build trainer with validation dataset
    try:
        # Extract trainer args
        trainer_args = dict(cfg.trainer.args)
        
        # Remove i_val and early_stopping from trainer_args as they're handled separately
        i_val = trainer_args.pop('i_val', 500)
        early_stopping = trainer_args.pop('early_stopping', {})
        
        trainer = DecoderOnlyFinetuneTrainer(
            model_dict,
            dataset,
            val_dataset=val_dataset,
            pretrained_decoder_path=cfg.get('pretrained_decoder'),
...
def main():
    ...
    parser.add_argument('--pretrained_decoder', type=str, default=None,
                        help='Optional: pretrained decoder checkpoint to start from')
    ...
    cfg.update(config)
    cfg.pretrained_decoder = args.pretrained_decoder
